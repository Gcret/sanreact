...
import { Empty, Input, Button, Radio, Steps, Checkbox, Icon } from 'antd';
...
export default class Cart extends Component {
				...
				return(
					...
					<div className={style.box}>
						...
						<div className={style.section}>
							
							{
								this.state.list.map((item,i)=>{
									...
									return(
											<div className={style.zil}>
													<p>行程名称:{item.pname}</p>
													<span className={style.jiages}>出行人数:{item.pnum}</span>&nbsp;&nbsp;&nbsp;
													<span className={style.jiages}>￥:{item.pprice}</span>
													<span className={style.jiages}><Button className={style.btn} type="danger" onClick={this.del.bind(this,i)}>删除</Button></span>
											</div>
										)
						...
						<div className={style.money}>
							...
							<div className={style.feiyong}><Icon type="file-sync" /> 费用明细</div>
							{
								this.state.list.map((item,i)=>{
									return(
										<div>
												
												<div className={style.jisuan}>
													人数:&nbsp;&nbsp;&nbsp;  {item.pnum}&nbsp;*&nbsp;<span className={style.qian}>￥&nbsp;&nbsp;&nbsp;</span>{item.pprice}
												</div>
							...
							<Link to="/index" className={style.shouye5}>点我返回首页</Link>
							<div className={style.zongji} onClick={this.tap.bind(this)}>点我获取总价:<span className={style.zongjia9}>{this.state.qian}</span>元</div>
						</div>
	...
	由旅游者的代理人订立合同的，代理人须确保获得相应代理权限，且代理人承诺将旅游合同约定及《行程单》相关内容如实完整告知旅游者。
				...
				return(
						<Empty 
				className={style.dianwo1}
				description={
				      <span>
				        暂无数据<span onClick={this.gouwus.bind(this)} className={style.dianwo}>点我去登录</span>
				      </span>
				    }
				/>
			)	
	...
	del(i){
		...
		let arr=this.state.list;
		api.getDelList({uid:200006,pid:id}).then((data)=>{
			arr.splice(i,1);
			this.setState({
				list:arr
			})
